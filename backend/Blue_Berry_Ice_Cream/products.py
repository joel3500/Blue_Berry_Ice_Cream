...
def _resolve_db_path():
    """
    1) En prod (Render) : on lit DB_PATH (défini dans render.yaml).
    2) En local : on utilise un fichier .db à côté de products.py.
    """
    db_path = os.getenv("DB_PATH")
    if db_path:
        # S'assure que le dossier existe (utile au 1er boot sur Render Disk)
        parent = os.path.dirname(db_path)
        if parent and not os.path.exists(parent):
            os.makedirs(parent, exist_ok=True)
        return db_path

    # Fallback local (dev) : fichier .db à côté du script
    here = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(here, "blue_blerry_ice_cream.db")

DB_PATH = _resolve_db_path()
db = SqliteDatabase(DB_PATH)
print(f"[BlueBerry] Using DB_PATH = {DB_PATH}")
...
db.create_tables([Persons, Cartes_Credits, Shipping_Informations, Products, Transactions, Produits, Orders], safe=True)
...
def inserer_le_JSON_du_Prof_ds_la_BD():
    ...
    local_data = extraction_des_donnees_JSON()   # ← va chercher le JSON ici
    produits = local_data["products"]
...
# Si cette donnée existe dejà dans la BD ONLINE, alors ça veut dire que le TOUTE LES DONNÉES SIMULÉES SIMULÉES Y SONT
# Les données API sont-elles déjà chargées ?

api_loaded = Products.select().exists()
if not api_loaded:
    print(" - Quelques insertions et mises à jour depuis l'API")
    inserer_le_JSON_du_Prof_ds_la_BD()
else:
    print(" - Les produits API existent déjà")
...
products = Flask(__name__)   # Remarque : Dans ce travail, on n'a pas bvesoin de Vues
...
@products.route("/index_1", methods=["GET", "POST"])
def index_1():
...
@products.route("/index_2", methods=["GET", "POST"])
def index_2():
    if request.method == 'POST':
        data_str = request.form.get("json_data")
        try:
...
@products.route("/index_3", methods=["GET", "POST"])
def index_3():
    if request.method == 'POST':
        data_str = request.form.get("json_data")
        try:
...
@products.route("/order/<int:order_id>/shipping", methods=["GET", "POST", "PUT"])
...
@products.route("/order/<int:order_id>/credit_card", methods=["GET", "POST", "PUT"])
...
# Étape décisive
@products.route("/order/<int:order_id>/facturation", methods=["GET", "POST", "PUT"]) 
...
@products.route("/origine_des_donnees")                                          
